...
def unblock_device(ip_address=None):
    # Get configuration from environment
    router_ip = os.getenv("ROUTER_IP", "192.168.1.1")
    api_path = os.getenv("ROUTER_API_PATH_UNBLOCK", "/api/unblock_device")
    ...
    try:
        ...
        print(f"Unblocked {ip_address}: Status {response.status_code}")
        return True
    except Exception as e:
        print(f"Failed to unblock device: {e}")
        return False
